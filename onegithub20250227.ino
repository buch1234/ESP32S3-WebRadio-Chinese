...
void fetchWeatherData() {
    ...
    if (httpCode == HTTP_CODE_OK) {
        ...
        if (!error) {
            ...
            else if (currentWeather.condition.indexOf(" réf") != -1) currentWeather.condition = "多云";
            ...
            for (int i = 0; i < 3; i++) {
                ...
                if (index < doc["list"].size()) {
                    ...
                    forecast[i].windDir = degToDirection(windDeg); // 转换为中文风向

                    if (forecast[i].condition.indexOf("clear") != -1) forecast[i].condition = "晴天";
                    else if (forecast[i].condition.indexOf("clouds") != -1) forecast[i].condition = "多云";
